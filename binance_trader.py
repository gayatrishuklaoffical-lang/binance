...
class BinanceTrader:
    ...
    def has_open_positions(self) -> bool:
        ...
        Returns:
            True if open positions exist, False otherwise
        """
        try:
            ...
            if open_positions:
                logger.warning(f"⚠️  {len(open_positions)} open position(s) detected:")
                for pos in open_positions:
                    symbol = pos['symbol']
                    amt = float(pos['positionAmt'])
                    entry = float(pos['entryPrice'])
                    pnl = float(pos['unRealizedProfit'])
                    logger.warning(f"   - {symbol}: {amt} @ ${entry:.2f} | PnL: ${pnl:.2f}")
                return True
            
            logger.info("✅ No open positions - proceeding with trade")
            return False
    ...
    def execute_trade(self, signal_data: Dict) -> bool:
        ...
        """
        try:
            # CRITICAL: Check if any positions are already open
            if self.has_open_positions():
                logger.error("🛑 TRADE BLOCKED: Existing position(s) must be closed first")
                logger.error("   Cannot open new trade while other positions are active")
                return False
